...
def preprocess_data(train_path="data/adult_train.csv", test_path="data/adult_test.csv"):
    """
    Učitava, čisti i preprocesira podatke 
    
    """
    # učitavanje i početno čišćenje
    ...
    combined_df.drop_duplicates(inplace=True) #brisem duplikate

    for col in combined_df.select_dtypes(include='object').columns:
        combined_df[col] = combined_df[col].astype(str).str.strip().str.replace('.', '', regex=False) #izbacujem .
        combined_df[col].replace('?', np.nan, inplace=True) #menjam ? sa NaN
    ...
    # uklanjanje redova sa NaN vrednostima u ciljnoj varijabli i uklanjanje retkih klasa
    valid_mask = y.notna()
    X = X[valid_mask].copy()
    y = y[valid_mask].copy()

    class_counts = y.value_counts()
    single_member_classes = class_counts[class_counts < 2].index #nalazim klase koje imaju samo 1 clan
    
    if not single_member_classes.empty:
        valid_mask_split = ~y.isin(single_member_classes) #pravim masku redova koje imaju vise od jednog clana
        X = X[valid_mask_split].copy()
        y = y[valid_mask_split].copy()
    
    # stratifikovana podela na trening i test set
    ...
    # odvajanje numeričkih i kategoričkih kolona
    ...
    # obrada outliera u numeričkim kolonama
    ...
    # normalizacija numeričkih kolona
    scaler = StandardScaler() # 0 sa snadradno devijacijom 1, normalizujemo na raspon -1 do 1 
    ...
    enc = OneHotEncoder(sparse_output=False, handle_unknown='ignore') #ako naidjes na neku nepozantu vr - ignorisi stavi 0
